...
*开始生效定时器1
*********************************************/
static void tq210_beep_start(void)
{
	unsigned long tcon;
	unsigned char i;
...
static void tq210_beep_off(void)
{
        unsigned long tcon;
        int err;
...
static void tq210_set_timer1(unsigned long Val)
{
	unsigned long tcon;//用于存放时钟控制寄存器的数值
	unsigned long tcnt;//用于存放TCNTB1的数值
	unsigned long tcmp;//用于存放TCMPB1的数值
    unsigned long tcfg1;//用于存放定时器配置寄存器1的数值，设置分割值
    unsigned long tcfg0;//用于存放定时器配置寄存器0的数值，设置预分频数值
	unsigned char i;
	...
	*/
    ...
    //设置分割值为1/1
        tcfg1 |= S3C2410_TCFG1_MUX1_DIV2;//set [4:7]== 1/1
	//清零预分频位TCFG0[0:7]
...
/*接口注册*/
static struct file_operations tq210_beep_fops=
{
	.owner			=THIS_MODULE,
